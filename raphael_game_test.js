...
//            console.log(this.matrix);
            ...
            this.rElmt.push(
                paper.rect(this.offsetToSvg.x,this.offsetToSvg.y,BOARD_SIZE.xGrids * BOARD_SIZE.gridWidth,BOARD_SIZE.yGrids * BOARD_SIZE.gridWidth).attr({fill:"#41bea8",stroke:'none'   /*,"fill-opacity": 0.5*/})
            );
//            绑定给raphael的click函数
...
//                console.log(event);
                    ...
                    if(this.TZBindObj.relatedText){
                        ...
                        this.TZBindObj.holder.push(this.TZBindObj.relatedText.rElmt);
                    }
...
//                this.delete(positionOrigin);  //这样写对么？
                    var newPos = getPointInGrid(this.center);
                    if(tianzi.board.matrix[newPos.gridX][newPos.gridY]){ //已经被占据
                        //todo
                        this.animate({x:this.positionOrigin.x ,y: this.positionOrigin.y},300); //square放回旧位置
                        ...
                        //todo
                        this.animate({x:newPos.gridX * tianzi.boardOption.gridWidth * tianzi.scale,
                            y:newPos.gridY * tianzi.boardOption.gridWidth * tianzi.scale},200);//square移到新位置
                        this.TZBindObj.status.gridX = newPos.gridX ; //刷新status
                        this.TZBindObj.status.gridY = newPos.gridY ;
                        //刷新matrix
                        tianzi.board.matrix[newPos.gridX][newPos.gridY] = {sqrId:this.TZBindObj.sqrId};
