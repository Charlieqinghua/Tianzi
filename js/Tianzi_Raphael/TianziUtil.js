...
function initRiddles(data){
    var riddles = data;
    _.each(riddles,function(item,idx){
        var startX = parseInt(item.startX),
            len = parseInt(item.len),
            startY = parseInt(item.startY),
            dire = item.dir;
        var rdl = new Tianzi.Frame({startGridX:startX , startGridY:startY , length:len , direction:dire});
        for(var i=0;i<len;i++){
            if(dire=='H'){
                var sqr = new Tianzi.Square({gridX:startX+i,gridY:startY});
            }
            else{
                var sqr = new Tianzi.Square({gridX:startX,gridY:startY + i});
            }
            rdl.contents.push({idx:i,squareId:sqr.squareId});
        }
        var a = tianzi.findElmentById('square',2);
        console.log(a);
    });
